...
def print_results_for_grading(engine, n=5):
    ...
    for query, gt_ids in GROUND_TRUTH.items():
        ...
        for method, results in [
            ...
            ("rrf",      engine.rrf_search(query, n=n)),
        ]:
            print(f"\n  {method}:")
...
def evaluate(engine, n=5):
    """Compute nDCG@5, Precision@5, and Recall@5 for BM25, semantic, and RRF across all queries."""
    all_ndcg      = {"bm25": [], "semantic": [], "rrf": []}
    all_precision = {"bm25": [], "semantic": [], "rrf": []}
    all_recall    = {"bm25": [], "semantic": [], "rrf": []}

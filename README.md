...
python evaluate.py
```
Retrieves the top-5 results per query for BM25 and semantic search and prints them with grading prompts. Fill in the relevance scores (0/1/2) in the `MANUAL_SCORES` dict in `evaluate.py`, then run again to compute nDCG@5, Precision@5, and Recall@5. Ground-truth books are automatically scored as 2.
